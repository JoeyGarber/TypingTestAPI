...
// pull in error types and logic to handle them and set status codes
...
router.post('/sign-in', (req, res, next) => {
  ...
  .then(record => {
    // if we don't find a user with that email, send 401
    if (!record) {
      throw new BadParamsError()
    }
  ...
  .then(correctPassword => {
    // if the passwords matched, making this true
    if (correctPassword) {
...
router.patch('/change-password', requireToken, (req, res, next) => {
  ...
  User.findById(req.user.id)
    // save user outside of promise chain
    .then(record => { user = record })
    // check that the old password is correct
    .then(() => bcrypt.compare(req.body.passwords.old, user.hashedPassword))
    // 'correctPassword' will be true if hashing the old password ends up the same
    // as 'user.hashedPassword'
    .then(correctPassword => {
      // throw an error if the new password is missing, an empty string,
      // or if the old password was wrong
...
// SIGN OUT
// DELETE /sign-out
router.patch('/sign-out', requireToken, (req, res, next) => {
  // nullify the existing token for the user
  req.user.token = null
